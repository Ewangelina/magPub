...
def log_csv(dataset_name, kan_real_grid, kan_grid, kan_degree, kan_lamb, kan_width, kan_params, kan_time, kan_test_acc, mlp_test_acc, mlp_time, mlp_params, mlp_layers, mlp_width, mlp_test_acc2, mlp2_time_difference, mlp_params2, no_layers2, width_layers2):
    csv_file = LOG_FILE_FILEPATH[:-4] + "_csv.txt"
    line = dataset_name + ";" + kan_real_grid + ";" + kan_grid + ";" + kan_degree + ";" + kan_lamb + ";" + kan_width + ";"  + kan_params + ";" + kan_time + ";" + kan_test_acc + ";" + mlp_test_acc + ";" + mlp_time + ";" + mlp_params + ";" + mlp_layers + ";" + mlp_width + ";" + mlp_test_acc2 + ";" + mlp2_time_difference + ";" + mlp_params2 + ";" + no_layers2 + ";" + width_layers2 + '\n'
...
def make_mlp_model_max_layers_delme(in_dim, no_layers):
  layers = []
  for i in range(no_layers):
...
def create_matching_mlp_max_layers_delme(KAN_width, KAN_degree, KAN_grid):
  kan_sum = 0
  for i in range(len(KAN_width) - 1):
    kan_sum += count_kan_layer_parameters(KAN_width[i], KAN_width[i+1], KAN_degree, KAN_grid)
...
def processKerasDatasetMNIST(random_state=None):
    ...
    with open('./data/extracted_test.sav', 'rb') as f:
        X_test_full, y_test_full = pickle.load(f)
...
log_csv("dataset_name", "kan_grid", "kan_grid_passed_parameter", "kan_degree", "kan_lam", "kan_width", "kan_params", "kan_time_difference", "kan_test_acc", "mlp_test_acc", "mlp_time_difference", "mlp_params", "mlp_no_layers", "mlp_width_layers", "mlp_test_acc2", "mlp2_time_difference", "mlp_params2", "no_layers2", "width_layers2")
            ...
            for width in KAN_width_table:
              for i in range(3):
                ...
                for l in range(len(width)-1):
                  kan_params += count_kan_layer_parameters(width[l], width[l+1], degree, accurate_grid_elements)
                ...
                kan_params2 = 0
                for l in range(len(width)-1):
                  kan_params2 += count_kan_layer_parameters(width[l], width[l+1], degree, accurate_grid_elements)
