...
write_to_file(f"This is KAN with {KAN_CNN_MNIST_WIDTH_ORIGIN} parameters (global pulling) and only one layer on CPU - batch size {BATCH_SIZE}. Optimizer is Adam")
...
class CNNKAN(nn.Module):
    def __init__(self, width1, grid1, k1, width2, grid2, k2):
        ...
        self.kan1 = KAN(width=width_copy, grid=grid1, k=k1, device=device, sparse_init=False)
...
def evaluate_with_log(model, test_data, train_data):
    test_loss, test_acc = evaluate(model, test_data)
    train_loss, train_acc = evaluate(model, train_data)

    line = "-Eval_results- Test loss: " + str(test_loss) + " Test accuracy: " + str(test_acc) + " Train loss: " + str(train_loss) + " Train accuracy: " + str(train_acc)
    write_to_file(line)
...
def train(train_data, test_data, model, steps=1):
    ...
    optimizer = optim.AdamW(model.parameters(), lr=0.001, weight_decay=1e-3) #<-------------------------------- zmień
    ...
    for i in range(steps):
        ...
        if evaluate_delay == 0:
          evaluate_delay = DELAY_EVALUATE
          accuracy = evaluate_with_log(model, test_data, train_data)
...
log_csv(["dataset_name", "accurate kan grid", "kan_grid 1", "kan_degree 1", "kan_lam 1", "kan_width 1", "kan_params", "kan_time_difference", "kan_steps", "kan_train_acc", "kan_val_acc", "kan_test_acc", "mlp_test_acc", "mlp_val_acc", "mlp_train_acc", "mlp_steps", "time_difference mlp1", "mlp_params", "width_layers", "mlp2_test_acc", "mlp2_val_acc", "mlp2_train_acc", "mlp2_steps", "mlp 2 time difference", "mlp2_params", "width_layers2", "KAN_CNN_MNIST_WIDTH_ORIGIN"])
        ...
        for lam in KAN_lambda_table:
            for width in KAN_width_table:
              for i in range(5):
                ...
                intro_line = "------------- start KAN ---- grid: " + str(grid) + " deg: " + str(degree) + " lambda: " + str(lam) + " width: " + str(width) + " DT: " + kan_start
                ...
                accurate_grid_elements = write_model_parameters(model.kan1)
                intro_line = "------------- MLP min start --- " + mlp_start
                write_to_file(intro_line)

                mlp_model, width_layers, kan_params = create_matching_mlp(width_copy, degree, accurate_grid_elements, False)
                mlp_model, mlp_val_acc, mlp_steps = train(train_subset, val_subset, mlp_model, STEPS)
                write_to_file("MLP min results")
                ...
                intro_line = "------------- MLP max start --- " + mlp_start
                write_to_file(intro_line)

                mlp2_model, width_layers2, kan_parameters = create_matching_mlp(width_copy, degree, accurate_grid_elements, True)
                mlp2_model, mlp2_val_acc, mlp2_steps = train(train_subset, val_subset, mlp2_model, STEPS)
                write_to_file("MLP max results")
                ...
                log_csv([dataset_name, accurate_grid_elements, grid, degree, lam, width_copy, kan_params, time_difference(kan_start, kan_end), steps, kan_train_acc, kan_val_acc, kan_test_acc, mlp_test_acc, mlp_val_acc, mlp_train_acc, mlp_steps, time_difference(mlp_start, mlp_end), mlp_params, width_layers, mlp2_test_acc, mlp2_val_acc, mlp2_train_acc, mlp2_steps, time_difference(mlp_start2, mlp_end2), mlp2_params, width_layers2, KAN_CNN_MNIST_WIDTH_ORIGIN])
