...
PATIENCE_VALUE = 3
TOLERANCE_AMOUNT = 0
LOG_FILE_FILEPATH = '.\\text_logs\\' + str(datetime.datetime.now()).replace(":", "").replace("-", "").replace(".", "") + '.txt'
STEPS = 1
    ...
    for degree in KAN_degree_table:
        for lam in KAN_lambda_table:
            for width in KAN_width_table:
                intro_line = "------------- start ---- grid: " + str(grid) + " deg: " + str(degree) + " lambda: " + str(lam) + " width: " + str(width)
                ...
                first_train_line = "normal train: " + str(last_i) + " steps; best val acc: " + str(best_test_acc)
                write_to_file(first_train_line)
                write_model_parameters(model)

                write_to_file("Turning interpretable ------------------")
                model.prune()
                model.auto_symbolic()
                ...
                second_train_line = "intepretable train: " + str(last_i) + " steps; best val acc: " + str(best_test_acc)
