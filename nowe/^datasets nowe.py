...
MLP_TEST_IN = 1
DELAY = 1
NO_MODELS_FOR_AVERAGE = 5

KAN_width_table = []
KAN_grid_table = [1, 2]
KAN_degree_table = [1, 2]
KAN_lambda_table = [0]
...
def trainKAN(dataset, steps=1, KAN_width=None, KAN_grid=None, KAN_degree=3, KAN_lambda=0, KAN_sparse_init=False, last_best_acc=0, model=None):
    ...
    if model is None:
        model = KAN(width=KAN_width, grid=KAN_grid, k=KAN_degree, device=device, sparse_init=KAN_sparse_init, auto_save=False,  grid_range=[0,1], affine_trainable = False)
    ...
    for i in range(steps):
        fit_results = model.fit(dataset, opt='Adam', steps=1, lamb=KAN_lambda, metrics=(train_acc, test_acc), loss_fn=torch.nn.CrossEntropyLoss(), batch=BATCH_SIZE, update_grid=False, grid_update_num = 1, stop_grid_update_step = 1, singularity_avoiding = True)
...
print(f"Widths used: {KAN_width_table}")
        ...
        for lam in KAN_lambda_table:
            for width in KAN_width_table:
              ...
              for i in range(NO_MODELS_FOR_AVERAGE):
                ...
                #-------------------MLP with only 1 hidden layer section -----------------------------

                mlp3_model, no_layers3, width_layers3 = create_matching_mlp(copied_width, degree, accurate_grid_elements, False, 1)
                mlp3_model, mlp3_val_acc, mlp3_steps = trainMLP(dataset, mlp3_model)
                mlp3_train_acc = mlp_evaluate(mlp3_model, dataset['train_input'], dataset['train_label'])
                mlp3_test_acc = mlp_evaluate(mlp3_model, full_dataset['test_input'], full_dataset['test_label'])

                mlp_end3 = string_datetime()
                line = "MLP 1 layer with " + str(no_layers3) + " layers of width " + str(width_layers2) + " achieved TEST accuracy of " + str(mlp3_test_acc) + " val acc: " + str(mlp3_val_acc) + " train acc: " + str(mlp3_train_acc) + " at " + str(mlp3_steps)
                write_to_file(line)

                mlp3_params = 0
                for l in range(no_layers3):
                  mlp3_params += count_mlp_layer_parameters(width_layers3, width_layers3)
                mlp3_params += count_mlp_layer_parameters(copied_width[0], width_layers3) + count_mlp_layer_parameters(width_layers3, copied_width[-1])

                log_csv([dataset_name, str(accurate_grid_elements), str(grid), str(degree), str(lam), str(copied_width), str(kan_params), time_difference(kan_start, kan_end), last_i, kan_train_acc, kan_val_acc, str(kan_test_acc), str(mlp_test_acc), mlp_val_acc, mlp_train_acc, mlp_steps, time_difference(mlp_start, mlp_end), str(mlp_params), str(no_layers), str(width_layers), str(mlp2_test_acc), mlp2_val_acc, mlp2_train_acc, mlp2_steps, time_difference(mlp_start2, mlp_end2), str(mlp2_params), str(no_layers2), str(width_layers2), str(mlp3_test_acc), mlp3_val_acc, mlp3_train_acc, mlp3_steps, time_difference(mlp_end2, mlp_end3), str(mlp3_params), str(no_layers3), str(width_layers3)])
                print(f"Accual grid: {accurate_grid_elements}")
