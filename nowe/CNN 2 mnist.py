...
write_to_file(f"This is KAN with {KAN_CNN_MNIST_WIDTH_ORIGIN} parameters (global pulling) and only one layer on local - batch size {BATCH_SIZE}. Optimizer is SGD")
...
def train(train_data, test_data, model, steps=1):
    ...
    optimizer = optim.SGD(model.parameters(), lr=0.001, weight_decay=1e-3) #<-------------------------------- zmień
